...
// Si ya está logeado, redirigir al panel
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id, username, email, password, es_desarrollador, nivel FROM usuarios WHERE username = :username");
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                $_SESSION['es_desarrollador'] = $user['es_desarrollador'];
                $_SESSION['nivel'] = $user['nivel'];
                $_SESSION['login_time'] = time();

                // Recordar sesión
                if ($remember) {
                    setcookie(COOKIE_NAME, $_SESSION['user_id'], time() + (86400 * 30), "/");
                }
            ...
            $error = DEBUG_MODE ? $e->getMessage() : 'Error al procesar el login';
        }
    }
}

// Verificar cookie "Recuérdame"
if (!isset($_SESSION['user_id']) && isset($_COOKIE[COOKIE_NAME])) {
    try {
        $stmt = $pdo->prepare("SELECT id, username, email, password, es_desarrollador, nivel FROM usuarios WHERE id = :id");
        $stmt->execute([':id' => $_COOKIE[COOKIE_NAME]]);
        $user = $stmt->fetch();
        ...
        <div class="login-card">
            ...
            <form method="POST" action="login.php">
                ...
                <div class="form-group checkbox">
                    ...
                    <label for="remember">Recuérdame en este equipo</label>
                </div>
